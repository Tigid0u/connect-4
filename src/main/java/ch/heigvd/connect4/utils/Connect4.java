...
public class Connect4 {
    ...
    /**
     * Constructeur de la classe Connect4 qui prend comme paramètre la taille de la grille désirée
     ...
     * Permet de savoir s'il est en dehors de la grille, ou si la colonne voulue est déjà pleine.
     * @param column Index de la colonne
     * @return retourne true si le coup est valide
     */
    public boolean checkInput(int column){
        return column >= 0 && column < maxColumns && grid[column][maxRows-1] == 0; // dernière condition, permet de savoir si la colonne est pleine
    }
     ...
     *         NOTHING : Indique qu'il n'y a rien à signaler, le jeu continue de manière normale
     *         DRAW : Indique que la grille est pleine et qu'il n'y a aucun vainqueur
     */
    ...
    /**
     * Méthode qui permet de checker s'il y a une victoire à partir d'une position donnée
    ...
    private boolean checkWin(int column, int row, int player){
        ...
        for (int[] direction : directions) {
            int count = 1;
            // Pour la colonne et la ligne, on ajoute déjà une fois la direction, car la case ou la pièce a été posé, est déjà compté
    ...
    /**
     * Méthode qui compte le nombre de pièces d'un joueur donné dans une direction donné
     * jusqu'à rencontrer une pièce d'un joueur adverse ou jusqu'à arriver à la limite de la grille
    ...
    /**
     * Méthode qui permet d'avoir la grille du jeu, qui contient l'état du jeu en cours
